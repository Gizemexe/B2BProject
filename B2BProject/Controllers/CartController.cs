...
namespace EcommerceProject2.Controllers
{
    public class CartController : Controller
    {
        ...
        [HttpPost]
        public ActionResult AddToOrderCart(int productId, int amount, string size, string priceType)
        {
            int customerId = GetCustomerId();
            if (customerId != 0)
            {
                using (var entities = new B2BDbEntities())
                {
                    ...
                    decimal price = (decimal)(priceType == "unit" ? product.UnitPrice : product.PackagePrice);
